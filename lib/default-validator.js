...
module.exports.validate = function validate(configObject, options={}) {
  ...
  if (config.extauth && config.extauth.hasOwnProperty('gracePeriod')) {
    assert(typeof config.extauth.gracePeriod === 'number', 'config.extauth.gracePeriod is not a number');
    assert(config.extauth.gracePeriod > 0, 'config.extauth.gracePeriod should be greater than zero');
  }
